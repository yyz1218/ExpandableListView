...
public class SimpleExpandActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        SimpleExpandableListAdapter adapter = new SimpleExpandableListAdapter(this, initGroupData(),
                R.layout.item_expand_group_normal, new String[]{Constant.GIRL_NAME}, new int[]{R.id.label_group_normal},
                initChildData(), R.layout.item_expand_child, new String[]{Constant.DETAILS_NAME}, new int[]{R.id.label_expand_child});
        ...
        listView.setOnChildClickListener(new ExpandableListView.OnChildClickListener() {
            ...
            public boolean onChildClick(ExpandableListView parent, View v, int groupPosition, int childPosition, long id) {
                Toast.makeText(SimpleExpandActivity.this, Constant.DETAILS[groupPosition][childPosition], Toast.LENGTH_SHORT).show();
                return true;
            }
        });
    }
    ...
    private List<Map<String, String>> initGroupData() {
        ...
        for (int i = 0; i < Constant.GirlS.length; i++) {
            map = new HashMap<>();
            map.put(Constant.GIRL_NAME, Constant.GirlS[i]);
            list.add(map);
        }
        return list;
    }
    ...
    private List<List<Map<String, String>>> initChildData() {
        ...
        Map<String, String> map;
        int row = Constant.DETAILS.length;
        int column = Constant.DETAILS[0].length;
        for (int i = 0; i < row; i++) {
            child = new ArrayList<>();
            for (int j = 0; j < column; j++) {
                map = new HashMap<>();
                map.put(Constant.DETAILS_NAME, Constant.DETAILS[i][j]);
                child.add(map);
            }
            list.add(child);
        }
